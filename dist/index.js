...
var SerialTransport = class extends EventEmitter4 {
   ...
   */
  start = async () => {
    logger3.info("SerialTransport", `\u542F\u52A8\u4E32\u53E3\u4F20\u8F93\u5C42: ${this.config.serialPath}`);
    await this.connect();
  };
  /**
   * 停止串口传输层
   */
  stop = async () => {
    logger3.info("SerialTransport", "\u505C\u6B62\u4E32\u53E3\u4F20\u8F93\u5C42");
  ...
  send = (data) => {
    if (!this.isConnected || !this.port) {
      logger3.warn("SerialTransport", "\u4E32\u53E3\u672A\u8FDE\u63A5\uFF0C\u65E0\u6CD5\u53D1\u9001\u6570\u636E");
      return;
    }
    try {
...
`;
      this.port.write(dataWithNewline, (err) => {
        if (err) {
          logger3.error("SerialTransport", "\u53D1\u9001\u6570\u636E\u5931\u8D25:", err);
          this.emit("error", `\u53D1\u9001\u6570\u636E\u5931\u8D25: ${err.message}`);
        } else {
          logger3.debug("SerialTransport", "\u53D1\u9001\u6570\u636E:", data);
        }
      });
    } catch (error) {
      logger3.error("SerialTransport", "\u53D1\u9001\u6570\u636E\u5F02\u5E38:", error);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.port.on("open", () => {
          ...
          logger3.info("SerialTransport", `\u4E32\u53E3\u8FDE\u63A5\u6210\u529F: ${this.config.serialPath}`);
          resolve();
        });
        this.port.on("error", (err) => {
          logger3.error("SerialTransport", "\u4E32\u53E3\u9519\u8BEF:", err);
          this.isConnected = false;
          this.emit("error", `\u4E32\u53E3\u9519\u8BEF: ${err.message}`);
          reject(err);
        });
        this.port.on("close", () => {
          logger3.warn("SerialTransport", "\u4E32\u53E3\u8FDE\u63A5\u5173\u95ED");
          this.isConnected = false;
          this.scheduleReconnect();
        });
        this.port.on("data", (data) => {
          logger3.debug("SerialTransport", "\u63A5\u6536\u539F\u59CB\u6570\u636E:", data.toString("utf8"));
        });
        this.parser.on("data", (data) => {
          logger3.debug("SerialTransport", "\u63A5\u6536\u89E3\u6790\u5206\u9694\u7B26\u540E\u7684\u6570\u636E:", data);
          try {
            this.handleReceivedData(data);
          } catch (error) {
            logger3.error("SerialTransport", "\u5904\u7406\u63A5\u6536\u6570\u636E\u5931\u8D25:", error);
            this.emit("error", `\u5904\u7406\u63A5\u6536\u6570\u636E\u5931\u8D25: ${error instanceof Error ? error.message : String(error)}`);
          }
        });
        this.port.open();
      } catch (error) {
        logger3.error("SerialTransport", "\u521B\u5EFA\u4E32\u53E3\u8FDE\u63A5\u5931\u8D25:", error);
        reject(error);
      }
    });
  }
  ...
  async disconnect() {
    return new Promise((resolve) => {
      if (this.port && this.isConnected) {
        this.port.close(() => {
          this.isConnected = false;
          logger3.info("SerialTransport", "\u4E32\u53E3\u8FDE\u63A5\u5DF2\u65AD\u5F00");
          resolve();
        });
      } else {
        resolve();
  ...
  handleReceivedData(data) {
    ...
    try {
      ...
      if (!requestPayload) {
        logger3.warn("SerialTransport", "\u63A5\u6536\u5230\u7684\u6570\u636E\u683C\u5F0F\u4E0D\u6B63\u786E:", data);
  ...
  scheduleReconnect() {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      logger3.error("SerialTransport", `\u91CD\u8FDE\u5931\u8D25\uFF0C\u5DF2\u8FBE\u5230\u6700\u5927\u91CD\u8FDE\u6B21\u6570: ${this.maxReconnectAttempts}`);
      return;
    }
    if (this.reconnectTimer) {
      return;
    ...
    logger3.info("SerialTransport", `${this.reconnectInterval / 1e3}\u79D2\u540E\u5C1D\u8BD5\u91CD\u8FDE (${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
    this.reconnectTimer = setTimeout(async () => {
      this.reconnectTimer = null;
      try {
        await this.connect();
        logger3.info("SerialTransport", "\u91CD\u8FDE\u6210\u529F");
      } catch (error) {
        logger3.error("SerialTransport", "\u91CD\u8FDE\u5931\u8D25:", error);
        this.scheduleReconnect();
      }
...
async function main() {
  ...
  if (!validation.valid) {
    logger4.error("Main", "\u914D\u7F6E\u9A8C\u8BC1\u5931\u8D25:");
    validation.errors.forEach((error) => logger4.error("Main", `  - ${error}`));
    process2.exit(1);
  }
  const deviceConfigs = configManager2.getDeviceConfigs();
  if (deviceConfigs.length === 0) {
    logger4.error("Main", "\u6CA1\u6709\u542F\u7528\u7684\u8BBE\u5907\u914D\u7F6E");
    process2.exit(1);
  }
  logger4.info("Main", `\u52A0\u8F7D\u4E86 ${deviceConfigs.length} \u4E2A\u8BBE\u5907\u914D\u7F6E:`);
  deviceConfigs.forEach((device) => {
    logger4.info("Main", `  - ${device.deviceId}: ${device.serialPath}`);
  });
  ...
  if (transportConfig.type === "http") {
    ...
    logger4.info("Main", `\u4F7F\u7528 HTTP \u4F20\u8F93\u5C42\uFF0C\u7AEF\u53E3: ${transportConfig.port}`);
  } else if (transportConfig.type === "serial") {
    transport = new SerialTransport(transportConfig);
    logger4.info("Main", `\u4F7F\u7528\u4E32\u53E3\u4F20\u8F93\u5C42\uFF0C\u7AEF\u53E3: ${transportConfig.serialPath}`);
  } else {
    logger4.error("Main", "\u4E0D\u652F\u6301\u7684\u4F20\u8F93\u5C42\u7C7B\u578B:", transportConfig.type);
    process2.exit(1);
  }
  ...
  transport.on("error", (error, cb) => {
    logger4.error("Main", "\u4F20\u8F93\u5C42\u9519\u8BEF:", error);
    cb(createErrorResponse(error));
  });
  deviceManager.on("device", (device) => {
    logger4.info("Main", "\u8BBE\u5907\u4E0A\u62A5:", device);
    const event = createDeviceEvent(device);
    transport?.send(event);
  });
  deviceManager.on("deviceConnected", (info) => {
    logger4.info("Main", `\u8BBE\u5907 ${info.deviceId} (${info.serialPath}) \u8FDE\u63A5\u6210\u529F`);
  });
  deviceManager.on("deviceDisconnected", (info) => {
    logger4.warn("Main", `\u8BBE\u5907 ${info.deviceId} (${info.serialPath}) \u65AD\u5F00\u8FDE\u63A5`);
  });
  deviceManager.on("deviceError", (error) => {
    logger4.error("Main", `\u8BBE\u5907 ${error.deviceId} (${error.serialPath}) \u53D1\u751F\u9519\u8BEF:`, error.error);
  });
  try {
    await transport.start();
    logger4.info("Main", "\u4F20\u8F93\u5C42\u542F\u52A8\u6210\u529F");
    await deviceManager.initializeDevices();
    const stats = deviceManager.getConnectionStats();
    logger4.info("Main", `\u8BBE\u5907\u521D\u59CB\u5316\u5B8C\u6210: ${stats.connected}/${stats.total} \u4E2A\u8BBE\u5907\u8FDE\u63A5\u6210\u529F`);
    if (stats.reconnecting > 0) {
      logger4.info("Main", `${stats.reconnecting} \u4E2A\u8BBE\u5907\u6B63\u5728\u91CD\u8FDE\u4E2D`);
    }
    if (stats.connected === 0 && stats.reconnecting === 0) {
      logger4.error("Main", "\u6CA1\u6709\u8BBE\u5907\u8FDE\u63A5\u6210\u529F\uFF0C\u7A0B\u5E8F\u9000\u51FA");
      process2.exit(1);
    }
    startHeartbeat();
    logger4.info("Main", "\u5FC3\u8DF3\u5B9A\u65F6\u5668\u5DF2\u542F\u52A8");
  } catch (error) {
    logger4.error("Main", "\u542F\u52A8\u5931\u8D25:", error);
    process2.exit(1);
  }
}
async function onReceiveStart(requestData) {
  ...
  logger4.info("Main", "\u6536\u5230\u542F\u52A8\u626B\u63CF\u6307\u4EE4", { rssi });
  try {
    await deviceManager?.startScan(rssi);
    logger4.info("Main", "\u6240\u6709\u8BBE\u5907\u5F00\u59CB\u626B\u63CF");
    return createStatusResponse({ msg: "Scan started" });
  } catch (error) {
    logger4.error("Main", "\u542F\u52A8\u626B\u63CF\u5931\u8D25:", error);
...
async function onReceiveStop() {
  ...
  try {
    ...
    logger4.info("Main", "\u6240\u6709\u8BBE\u5907\u505C\u6B62\u626B\u63CF");
    return createStatusResponse({ msg: "Scan stopped" });
  } catch (error) {
    logger4.error("Main", "\u505C\u6B62\u626B\u63CF\u5931\u8D25:", error);
...
process2.on("SIGINT", async () => {
  ...
  try {
    ...
    logger4.info("Main", "\u7A0B\u5E8F\u5DF2\u5B89\u5168\u5173\u95ED");
  } catch (error) {
    logger4.error("Main", "\u5173\u95ED\u7A0B\u5E8F\u65F6\u53D1\u751F\u9519\u8BEF:", error);
  }
  process2.exit();
});
main();
